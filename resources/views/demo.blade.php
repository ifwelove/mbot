...
{{--                <th scope="col">MAC</th>--}}
            ...
            @foreach ($machines as $index => $machine)
                <tr>
                    ...
                    <td>@foreach ($machine['rows'] as $status => $total){{ $status }}:{{ $total }}<br>@endforeach
...
{{--                        <button class="command-btn update-btn btn btn-danger" data-token="{{ $token }}" data-mac="{{ $machine['mac'] }}" data-command="update_mpro">更新大尾</button>--}}
                        ...
                        <button class="command-btn apk_install-btn btn btn-danger" data-token="{{ $token }}" data-mac="{{ $machine['mac'] }}" data-command="apk_install">安裝apk</button>
                        <button class="command-btn open_exception_auto_reboot-btn btn btn-danger" data-token="{{ $token }}" data-mac="{{ $machine['mac'] }}" data-command="open_exception_auto_reboot">開啟模擬器畫面異常自動重啟</button>
                        <button class="command-btn close_exception_auto_reboot-btn btn btn-danger" data-token="{{ $token }}" data-mac="{{ $machine['mac'] }}" data-command="close_exception_auto_reboot">關閉模擬器畫面異常自動重啟</button>
